...
[**_SvBench_**](#using) is a free, powerful benchmark that provides a series of algorithms for both exact computing and approximate computing of Shapley Value in data analytics.
...
As shown in the figure, **_SvBench_** is composed of a config loader, a sampler, a utility calculator, a convergence checker, and an output aggregator for computing SV by iterative rounds.
A round of SV calculation is conducted starting from the sampler and ending at the convergence checker.
If the convergence criterion is not met, another round will be initiated as demonstrated in the figure (with dashed arrow).
...
In this repository, we show three typical DA tasks, namely **Result Interpretation(RI)**, **Data Valuation(DV)**, and **Federated Learning(FL)**, as example use cases. The datasets used by each task (which are generated by `Task/data_preparation.py`) and the cooperative game modeling for each task are summarized in the following tables:
...
-   The RI task on Iris (RI_Iris) uses SV to explain how four biological features influence the results of classifying three types of iris plants.
-   The RI task on Wine (RI_Wine) uses SV to explain how 13 chemical constituents affect the results of classifying three types of wine.
...
-   The FL tasks on MNIST and Cifar-10 (FL_MNIST and FL_Cifar) distribute the two datasets to 10 devices, using SV to valuate the local models trained by those devices for higher accuracy in handwritten digit classification and object recognition.
...
Here, we create two new DV tasks on Iris and Wine, denoted as DSV_Iris and DSV_Wine, respectively. In the two tasks, the players are datasets, each containing 10% of tuples in the original dataset. 
We achieve the transformation from data tuple valuation to data set valuation by properly setting the parameter `--tuple_to_set`.
