...
This repository [introduces **_SvBench_**](#using), a benchmark framework for developing Shapley Value (SV) applications in the data analytics (DA) domain. We also provide [the instructions for the experiments](#Experiments) presented in [a comprehensive survey of SV in DA](https://arxiv.org/abs/2412.01460).
...
| **DV** | _Iris_     | _Multilayer Perceptron_        | data tuple （n=120）       | Test Accuracy | 
| **DV** | _Wine_     | _Multilayer Perceptron_        | data tuple （n=142）       | Test Accuracy | 
...
-   The RI task on Iris (RI_Iris) uses SV to explain how the four biological features influence the results of classifying three types of iris plants.
-   The RI task on Wine (RI_Wine) uses SV to explain how 13 chemical constituents influence the results of classifying three types of wine.
-   The DV tasks on Iris and Wine (DV_Iris and DV_Wine) use SV to evaluate the importance of 120 iris plants and 142 wine samples to improve the classification accuracy.
-   The FL tasks on MNIST and Cifar-10 (FL_MNIST and FL_Cifar) distribute the two datasets to 10 devices, using SV to valuate the local models trained by those devices for the higher accuracy in handwritten digits classification and object recognition.
...
|   convergence_threshold    |                      $0 - 1$                       | Threshold for determining the convergence and termination of SV computing algorithm. `0` for **EXACT** computing of SV. |   0.1    | float |
...
## Extend **_SvBench_**
**_SvBench_**  provides several APIs in its modules (marked by an API icon in the [figure](#overview)) for developers to extend this benchmark. One can configure the modules he expects to extend by his user-specific parameters and functions at the SV computation step. 
...
|  calculator     |     utility_function      |  function | The function for utility computation, which takes the `list` generated from the sampling function as the input and output a utility value representing the collective efforts of the players cincluded in the input list. |  
|  calculator     |     optimization_strategy      |  str | The user-specific strategy for optimizing efficiency of utility computation. Setting a User-specific optimization strategy is allowed only when the utility_function is set by user-specific function. |  
|  calculator     |  base_algo | function | The function for user-specific sv computing algorithm, which takes the outputs from the sampling function as its input and generates a set of utility computation results for output aggregator. |
|  convergence checker |   convergence_check    | function | The function to determine the termination of SV computing process, which takes several rounds of SV results as inputs and outputs a `boolean` value. | 
...
|  output aggregator | privacy_protection_measure | function | The function for handling SV-driven privacy issues. The function input and output are both a `dictionary` with the key being the identifier of each players, the value being the SV of each player. | 
...
(1) experiments on **RI_Iris** task
```
# FIA-U: SV-driven feature inference attack based on a uniform random distribution [the case without privacy protection]
...
python attack_exp.py --attack=MIA --task=DV --dataset=iris --base_algo=MC
...
```
# MIA: SV-driven membership inference attack [the case without privacy protection]
python attack_exp.py --attack=MIA --task=DV --dataset=wine --base_algo=MC
